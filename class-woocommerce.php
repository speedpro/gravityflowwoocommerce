	...
	class Gravity_Flow_Woocommerce extends Gravity_Flow_Extension {
		...
		public function update_entry( $order_id, $from_status, $to_status, $order ) {
			...
			foreach ( $entry_ids as $entry_id ) {
				...
				if ( $current_step && 'woocommerce_payment' === $current_step->get_type() && ( 'pending' === $from_status || 'on-hold' === $from_status ) && ( $to_status === apply_filters( 'gravityflowwoocommerce_payment_step_complete_status', $to_status ) ) ) {
					if ( true === $result ) {
